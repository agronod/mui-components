...
- `CHANGELOG.md` is regenerated by the release workflow on every publish (`npm run changelog -- -u` after the version bump, conventional-changelog `react` preset, `-r 0` rebuilds the whole file from tags). Nobody runs it by hand and manual edits are overwritten. Before 2026-09-17 it was a manual README step and went stale twice.
