...
`CHANGELOG.md` is generated by the release workflow, not by hand. On every publish (push to `main`, `feat/*` or `fix/*`) the workflow bumps the version, regenerates the whole file from git tags with the new version's commits on top, and commits it together with the version bump. Do not edit or commit `CHANGELOG.md` manually; the next release overwrites it.
